...
//EX::
void KdDirect3D::ConverWorldToScreenDatail(const KdCamera& camera, const Math::Vector3& pos, Math::Vector3& result)
{
	//ビューポート
	Math::Viewport vp;
	CopyViewportInfo(vp);

	//1ワールド変換行列xビュー行列x射影行列
	Math::Matrix world = Math::Matrix::CreateTranslation(pos);
	Math::Matrix wvp = world * camera.GetCameraMatrix().Invert() * camera.GetProjMatrix();
